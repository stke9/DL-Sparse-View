...
x_test, y_test = x[15985:, :, :256, :256], y[15985:, :, :256, :256]
x, y = x[:15985, :, :256, :256], y[:15985, :, :256, :256]
# x_rot, y_rot = x[:, :, :256, :256], y[:, :, :256, :256]
# for q in range(1,4):
#     x_rot = torch.cat((x_rot, torch.rot90(x, q, dims=(2, 3))[:, :, :256, :256]))
#     y_rot = torch.cat((y_rot, torch.rot90(y, q, dims=(2, 3))[:, :, :256, :256]))
print('Dataloading')
...
for epoch in range(15):
    for i, (x_train, y_train) in enumerate(train_data):
        ...
        if i % 30 == 0:
            ...
            one_hot_pred = torch.round(one_hot_pred)
            for ch in range(4):
            ...
            x_test_copy = x_test[:, 0].cuda()
            y_test_copy = y_test[:, 0].cuda()
            x_test_copy[one_hot_pred[:, 0] == 1] = 0
            x_test_copy[one_hot_pred[:, 1] == 1] = 0.194
            x_test_copy[one_hot_pred[:, 2] == 1] = 0.233
            x_test_copy[one_hot_pred[:, 3] == 1] = y_test_copy[one_hot_pred[:, 3] == 1]
            loss = loss_func(x_test_copy, y_test_copy)
            img = x_test_copy - y_test_copy
            wandb.log({'effective loss': loss.item()**0.5})
            print(epoch, i, loss.mean().item()**0.5)
            images = [im[j].cpu() for j in range(4) for im in [x_test, y_test, img]]
